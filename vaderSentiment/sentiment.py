...
class SentimentIntensityAnalyzer(object):
    ...
    def polarity_scores(self, text):
        ...
        for i, item in enumerate(words_and_emoticons):
            ...
            if (i < len(words_and_emoticons) - 1 and item.lower() == "kind" and
                ...
                sentiments.append(valence)
                continue
            length_text = len(words_and_emoticons)
            sentiments = self.sentiment_valence(valence, sentitext, item, i, sentiments, length_text)
    ...
    def sentiment_valence(self, valence, sentitext, item, i, sentiments, length_text):
...
if __name__ == '__main__':
    ...
    neg = 0
    mix = 0
    # For each comment, get the sentiment score and determine whether positive, negative or mixed sentiment
    for i in range(len(sentence)):
        ...
        temp_dict = {'tweet': s, 'senti_score': vs['compound'],'label': l}
        if vs['compound'] >= 0.05:
            temp_dict['senti_class'] = 'pos'
            pos+=1
        elif (vs['compound'] > -0.05 and vs['compound'] < 0.05):
            temp_dict['senti_class'] = 'mix'
            mix+=1
        else:
            temp_dict['senti_class'] = 'neg'
            neg+=1
        # master_dict[s] = temp_dict
        # master_dict[i] = temp_dict
        master_dict.append(temp_dict)

    # master_dict[s]['pos_score'] = pos
    # master_dict[s]['neg_score'] = neg
    # print(master_dict)
    print("total positive:", pos)
    print("total neg", neg)
    print("total mix", mix)

    with open('sentimentAnalysis.csv', mode='w') as csv_file:
        fieldnames = ['tweet', 'label', 'senti_score','senti_class']
    ...
    # # Read each video's JSON file and fetch all comments along with ids
    # for f in tqdm.tqdm(comment_files):
    #     comments_list = []
    #     sentence = []
    #     comment_id = []
    #     author = []
    #     timestamp = []
    #     video_id = f.split(".")[1]
    #     master_dict[video_id] = {}

    #     with open(f, 'r') as outfile:
    #         data = json.load(outfile)
    #         for p in data:
    #             sentence.append(p['snippet']['topLevelComment']['snippet']['textOriginal'])
    #             comment_id.append(p['snippet']['topLevelComment']['id'])
    #             author.append(p['snippet']['topLevelComment']['snippet']['authorDisplayName'])
    #             timestamp.append(p['snippet']['topLevelComment']['snippet']['updatedAt'])
    ...
    #         vs = analyzer.polarity_scores(s)
    #         total_score += vs['compound']
    #         temp_dict = {'text': s, 'senti_score': vs['compound'],'author':author[i], 'timestamp': timestamp[i]}
    #         if vs['compound'] >= 0.05:
    #             temp_dict['senti_class'] = 'pos'
    #             pos+=1
    #         elif (vs['compound'] > -0.05 and vs['compound'] < 0.05):
    ...
    #             temp_dict['senti_class'] = 'neg'
    #             neg+=1
    #         master_dict[video_id][cid] = temp_dict

    #     master_dict[video_id]['pos_score'] = pos
    #     master_dict[video_id]['neg_score'] = neg

        # # Get the likes, dislikes counts from the stats json file
        # likes = int(stats_data[video_id]['likeCount'])
        # dislikes = int(stats_data[video_id]['dislikeCount'])
        # master_dict[video_id]['likes'] = likes
        # master_dict[video_id]['dislikes'] = dislikes

    # # add all the acquired data to this file
    # with open("master_dict.csv", "w") as f:
    #     writer = csv.writer(f, delimiter=',')
    #     f.write(json.dumps(master_dict))
